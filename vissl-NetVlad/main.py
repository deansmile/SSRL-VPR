...
parser.add_argument('--dataPath', type=str, default='/home/ds5725/vissl-NetVlad/data', help='Path for centroid data.')
parser.add_argument('--runsPath', type=str, default='/home/ds5725/vissl-NetVlad/runs', help='Path to save runs to.')
parser.add_argument('--savePath', type=str, default='/home/ds5725/vissl-NetVlad/checkpoints', 
...
parser.add_argument('--arch', type=str, default='deepcluster', 
        help='basenetwork to use', choices=['vgg16', 'alexnet','simclr','jigsaw','deepcluster'])
        ...
        with h5py.File(train_set.cache, mode='w') as h5: 
            ...
            with torch.no_grad():
                for iteration, (input, indices) in enumerate(whole_training_data_loader, 1):
                    ...
                    image_encoding = model.encoder(input)[0]
...
def test(eval_set, epoch=0, write_tboard=False):
    ...
    fquery=open('test.txt','w')
    #TODO can we do this on the matrix in one go?
    for qIx, pred in enumerate(predictions):
        fquery.write(str(qIx)+' '+str(pred)+'\n')
    ...
    with h5py.File(initcache, mode='w') as h5: 
        with torch.no_grad():
            ...
            for iteration, (input, indices) in enumerate(data_loader, 1):
                input = input.to(device)
                image_descriptors = model.encoder(input)[0].view(input.size(0), encoder_dim, -1).permute(0, 2, 1)
...
if __name__ == "__main__":
    ...
    elif opt.arch.lower() == 'deepcluster':
        encoder_dim = 2048
        encoder = deepcluster.setModel()

    if opt.arch.lower() != 'deepcluster':
